...
exports.GraphQLTime = exports.parseTime = exports.RFC_3339_REGEX = void 0;
const graphql_1 = require("graphql");
const neo4j_driver_1 = __importDefault(require("neo4j-driver"));
exports.RFC_3339_REGEX = /^(?<hour>[01]\d|2[0-3]):(?<minute>[0-5]\d):(?<second>[0-5]\d)(\.(?<fraction>\d{1}(?:\d{0,8})))?((?:[Zz])|((?<offsetDirection>[-|+])(?<offsetHour>[01]\d|2[0-3]):(?<offsetMinute>[0-5]\d)))?$/;
const parseTime = (value) => {
    if (typeof value !== "string") {
        throw new TypeError(`Value must be of type string: ${value}`);
    }
    const match = exports.RFC_3339_REGEX.exec(value);
    if (!match) {
...
exports.GraphQLTime = new graphql_1.GraphQLScalarType({
    ...
    serialize: (value) => {
        ...
        if (!exports.RFC_3339_REGEX.test(stringifiedValue)) {
